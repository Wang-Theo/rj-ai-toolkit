...
"""
MSG 邮件解析模块
使用 extract-msg 库解析 MSG 文件，提取邮件内容并转换为 Markdown 格式
"""
...
class MSGParser(BaseParser):
    ...
    def parse_file(self, file_path: Union[str, Path]) -> str:
        ...
        # 如果未指定附件目录，使用MSG文件同目录下的attachments文件夹
        ...
        # 处理附件并保存
        embedded_images, attachment_files = self._save_attachments(msg, attachments_base_dir)
        
        # 获取邮件内容（Markdown 格式）
        content_markdown = self._extract_content_as_markdown(msg, embedded_images, attachments_base_dir)
    ...
    def _save_attachments(self, msg, attachments_base_dir: Path) -> tuple:
        """
        处理邮件附件，区分嵌入图片和附件文件
        根据 save_attachments 参数决定是否保存文件
        
        Args:
            msg: extract_msg 解析后的邮件对象
            attachments_base_dir: 附件保存基础目录
            
        Returns:
            tuple: (embedded_images, attachment_files)
                - embedded_images: dict {content_id: {'path': str, 'saved_path': Path or None}}
        ...
        for attachment in msg.attachments:
            try:
                ...
                # 判断是否是嵌入图片 (通过 contentId 或隐藏属性判断)
                content_id = getattr(attachment, 'contentId', None)
                is_hidden = getattr(attachment, 'hidden', False)
                is_embedded_image = bool(content_id) or (is_hidden and attachment_filename.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.bmp', '.tiff', '.webp')))
                ...
                if is_embedded_image:
                    if self.save_attachments:
                        ...
                        actual_path = self._save_attachment_file(attachment_data, saved_path, is_image=is_img)
                        
                        if actual_path:
                            # 记录实际保存路径（可能文件名被修改）
                            rel_path = f"{attachments_base_dir.name}/embedded/{actual_path.name}"
                            cid_key = content_id if content_id else attachment_filename
                            embedded_images[cid_key] = {
                                'path': rel_path,
                    ...
                    # 计算文件大小
                    file_size = len(attachment_data) if attachment_data else 0
    ...
    def _save_attachment_file(self, data: bytes, save_path: Path, is_image: bool = False):
        """
    ...
    def _extract_content_as_markdown(self, msg, embedded_images: dict, attachments_base_dir: Path) -> str:
        """
        ...
        content_parts.append(f"**Date:** {self._format_date(date)}  \n")
